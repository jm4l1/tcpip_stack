...
// structure for IP address in form
// xxx.xxx.xxx
typedef struct ip_add_ {
  char ip_addr[16];
} ip_add_t;

// structure for MAC address in from
//  xx:xx:xx:xx:xx:xx
...
static inline void init_node_nw_prop(node_nw_prop_t *node_nw_prop) {
  ...
  memset(node_nw_prop->lb_addr.ip_addr, 0,
         sizeof(node_nw_prop->lb_addr.ip_addr));
  init_arp_table(&(node_nw_prop->arp_table));
  init_mac_table(&(node_nw_prop->mac_table));
  init_rt_table(&(node_nw_prop->route_table));
}

typedef enum { ACCESS, TRUNK, L2_MODE_UNKNOWN } intf_l2_mode_t;
...
static inline void init_intf_nw_prop(intf_nw_props_t *intf_nw_props) {
  ...
  memset(intf_nw_props->ip_add.ip_addr, 0,
         sizeof(intf_nw_props->ip_add.ip_addr));
