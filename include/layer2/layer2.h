...
#define ETH_HDR_SIZE_EXCL_PAYLOAD                                              \
  (sizeof(ethernet_frame_t) - sizeof(((ethernet_frame_t *)0)->payload) - 4)
#define VLAN_ETH_HDR_SIZE_EXCL_PAYLOAD                                         \
  (sizeof(vlan_tagged_ethernet_frame_t) -                                      \
   sizeof(((vlan_tagged_ethernet_frame_t *)0)->payload) - 4)
#define ETH_FCS(eth_frame, payload_size)                                       \
  ((unsigned int *)(((char *)(((ethernet_frame_t *)eth_frame)->payload) +      \
                     payload_size)))
#define VLAN_ETH_FCS(vlan_tagged_eth_frame, payload_size)                      \
  ((unsigned int *)((                                                          \
      (char *)(((vlan_tagged_ethernet_frame_t *)vlan_tagged_eth_frame)         \
                   ->payload) +                                                \
      payload_size)))
#define ARP_PACKET_SIZE sizeof(arp_packet_t)
#define GET_COMMON_ETH_FCS(eth_frame, payload_size)                            \
  (is_pkt_vlan_tagged(eth_frame) ? VLAN_ETH_FCS(eth_frame, payload_size)       \
                                 : ETH_FCS(eth_frame, payload_size))
#pragma pack(push, 1)
...
typedef void (*arp_process_fun)(node_t *, interface_t *oif, arp_entry_t *,
                                arp_pending_entry_t *);
...
GLTHREAD_TO_STRUCT(arp_glue_to_arp_pending_entry, arp_pending_entry_t,
                   arp_pending_entry_glue);
...
static inline unsigned int get_8021q_vlan_id(vlan_8021q_tag_t *vlan_8021q_tag) {
  return vlan_8021q_tag->tci_vid;
}

static inline char *get_ethernet_frame_payload(ethernet_frame_t *eth_frame) {
...
static inline void set_common_eth_fcs(ethernet_frame_t *eth_frame,
                                      uint32_t payload_size, uint32_t new_fcs) {
...
ALLOC_ETH_HDR_WITH_PAYLOAD(char *pkt, unsigned int pkt_size) {
  ethernet_frame_t *header =
      (ethernet_frame_t *)(pkt - ETH_HDR_SIZE_EXCL_PAYLOAD);
...
//     is_pkt_vlan_tagged(ethernet_frame);
...
//     arp_packet_t* arp_packet = (arp_packet_t *)
//     get_ethernet_frame_payload(vlan_eth_frame);
...
static inline bool_t l2_frame_recv_qualify_on_interface(
    interface_t *intf, ethernet_frame_t *eth_frame, uint16_t *output_vlan_id) {
  ...
  if (!IS_INTF_L3_MODE(intf) && IF_L2_MODE(intf) == L2_MODE_UNKNOWN) {
    if (intf->att_node->debug_status == DEBUG_ON)
      printf("[l2_frame_recv_qualify_on_interface]Alert : %s - Interface : %s "
             "is in unknown mode\n",
  ...
  if (IS_INTF_L3_MODE(intf)) {
    ...
    if (vlan_eth_frame) {
      if (intf->att_node->debug_status == DEBUG_ON)
        printf("[l2_frame_recv_qualify_on_interface] Info : %s - Interface %s "
               "is in L3 mode cannot accept vlan tagged Frames\n",
               intf->att_node->node_name, intf->if_name);
      return FALSE;
    }
  }
  ...
  if (IF_L2_MODE(intf) == ACCESS) {
    ...
    if (vlan_eth_frame && (vlan_id != if_acces_vlan)) {
      printf("[l2_frame_recv_qualify_on_interface] Info : %s - Interface %s is "
             "not a member of VLAN %hu\n",
             intf->att_node->node_name, intf->if_name, vlan_id);
      return FALSE;
    }
    printf("[l2_frame_recv_qualify_on_interface] Info : %s - Interface %s is "
           "operating in Access mode, accept and tag frame with vlan id %hu\n",
  ...
  if (IF_L2_MODE(intf) == TRUNK) {
    ...
    if (!vlan_eth_frame) {
      printf("[l2_frame_recv_qualify_on_interface] Info : %s - Interface %s is "
             "operating in Trunk mode, can not accept untagged frames\n",
             intf->att_node->node_name, intf->if_name);
      return FALSE;
    }
    ...
    if (is_trunk_interface_vlan_member(intf, vlan_id)) {
      printf("[l2_frame_recv_qualify_on_interface] Info : %s - Interface %s is "
             "operating in Trunk mode, accept tagged frame with vlan id %hu\n",
             intf->att_node->node_name, intf->if_name, vlan_id);
      return TRUE;
    }
    printf("[l2_frame_recv_qualify_on_interface] Info : %s - Interface %s is "
  ...
  if (memcmp(IF_MAC(intf), dest_mac, sizeof((mac_addr_t *)0)->mac) == 0) {
    if (intf->att_node->debug_status == DEBUG_ON)
      printf("[l2_frame_recv_qualify_on_interface] Info : %s - Frame with MAC "
             "of %s\n",
             intf->att_node->node_name, intf->if_name);
    return TRUE;
  }
  ...
  if (IS_MAC_BROADCAST_ADDR(dest_mac)) {
    if (intf->att_node->debug_status == DEBUG_ON)
      printf("[l2_frame_recv_qualify_on_interface] Info : %s - Broadcast Frame "
             "received %x:%x:%x:%x:%x:%x \n",
             intf->att_node->node_name, eth_frame->src_mac.mac[0],
             eth_frame->src_mac.mac[1], eth_frame->src_mac.mac[2],
             eth_frame->src_mac.mac[3], eth_frame->src_mac.mac[4],
             eth_frame->src_mac.mac[5]);
    return TRUE;
  }
  if (intf->att_node->debug_status == DEBUG_ON)
    printf("[l2_frame_recv_qualify_on_interface] Info : %s - Dropping Frame "
           "with src mac %x:%x:%x:%x:%x:%x \n",
           intf->att_node->node_name, eth_frame->src_mac.mac[0],
           eth_frame->src_mac.mac[1], eth_frame->src_mac.mac[2],
           eth_frame->src_mac.mac[3], eth_frame->src_mac.mac[4],
           eth_frame->src_mac.mac[5]);
