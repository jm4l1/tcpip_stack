...
#define IP_HDR_PAYLOAD_SIZE(ip_hdr_ptr)                                        \
  (IP_HDR_TOTAL_LEN_IN_BYTES(ip_hdr) - IP_HDR_LEN_IN_BYTES(ip_hdr))
...
void send_icmp_request(node_t *node, uint16_t identifier, uint16_t seq_num,
                       uint32_t dest_ip);
