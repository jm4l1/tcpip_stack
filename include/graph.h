 ...
 *       Compiler:  gcc
 *
 *         Author:  Er. Abhishek Sagar, Networking Developer (AS),
 * sachinites@gmail.com Company:  Brocade Communications(Jul 2012- Mar 2016),
 * Current : Juniper Networks(Apr 2017 - Present)
 *
 *        This file is part of the NetworkGraph distribution
 * (https://github.com/sachinites). Copyright (c) 2017 Abhishek Sagar. This
...
typedef enum debug_status { DEBUG_ON, DEBUG_OFF } debug_status_t;
struct node_ {
...
static inline void set_node_debug_status(node_t *node, debug_status_t status) {
  node->debug_status = status;
  printf("Info : %s - debug %s\n", node->node_name, debug_status_str(status));
}
static inline node_t *get_node_by_node_name(graph_t *topology,
                                            char *node_name) {
  node_t *node;
  glthread_t *curr;
