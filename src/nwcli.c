...
extern graph_t *topology;

extern void send_arp_broadcast_request(node_t *node, interface_t *oif,
                                       char *ip_addr);
...
extern void rt_table_add_route(route_table_t *route_table, char *dst,
                               uint8_t mask, char *gw_ip, char *oif_name);
...
static int show_nw_topology_handler(
  ...
  case CMDCODE_SHOW_NW_TOPOLOGY:
    dump_nw_graph(topology);
    break;
  default:;
  }
  return 0;
}
static int show_saved_topologies_handler(
    param_t *param,           // parameter passed to handler call back
    ser_buff_t *tlv_buff,     // tlv structure described param
    op_mode enable_or_disable // is command enable or disable function
) {
...
static int show_node_interfaces(
    param_t *param,           // parameter passed to handler call back
    ser_buff_t *tlv_buff,     // tlv structure described param
    op_mode enable_or_disable // is command enable or disable function
) {
...
static int show_mode_handler(
  ...
  if (IS_INTF_L3_MODE(intf)) {
    ...
    printf("IF %s is in %s Mode\n", if_name,
           intf_l2_mode_str(IF_L2_MODE(intf)));
  }
  return 0;
}
static int show_vlans_handler(
    param_t *param,           // parameter passed to handler call back
    ser_buff_t *tlv_buff,     // tlv structure described param
    op_mode enable_or_disable // is command enable or disable function
) {
...
static int show_debug_handler(
  ...
  printf("Node %s debugging is %s\n", node->node_name,
         debug_status_str(node->debug_status));
  return 0;
}
static int config_route_handler(
    param_t *param,           // parameter passed to handler call back
    ser_buff_t *tlv_buff,     // tlv structure described param
    op_mode enable_or_disable // is command enable or disable function
) {
...
static int config_debug_handler(
    param_t *param,           // parameter passed to handler call back
    ser_buff_t *tlv_buff,     // tlv structure described param
    op_mode enable_or_disable // is command enable or disable function
) {
...
static int remove_vlan_handler(
    param_t *param,           // parameter passed to handler call back
    ser_buff_t *tlv_buff,     // tlv structure described param
    op_mode enable_or_disable // is command enable or disable function
) {
...
static int resolve_arp_handler(
    param_t *param,           // parameter passed to handler call back
    ser_buff_t *tlv_buff,     // tlv structure described param
    op_mode enable_or_disable // is command enable or disable function
) {
...
static int
ping_handler(param_t *param,           // parameter passed to handler call back
             ser_buff_t *tlv_buff,     // tlv structure described param
             op_mode enable_or_disable // is command enable or disable function
) {
  if (!topology) {
...
static int create_topology_handler(
    param_t *param,           // parameter passed to handler call back
    ser_buff_t *tlv_buff,     // tlv structure described param
    op_mode enable_or_disable // is command enable or disable function
) {
  ...
  TLV_LOOP_END;
  if (topology)
    memset(topology, 0, sizeof(graph_t));
  topology = create_new_graph(topology_name);
  printf("Topology Created.\n");
  return 0;
}
static int validate_topology_id(char *topology_id) {
...
static int validate_debug_status(char *status) {
  if (strcmp(status, "on") == 0 || strcmp(status, "ON") == 0 ||
      strcmp(status, "off") == 0 || strcmp(status, "OFF") == 0)
    return VALIDATION_SUCCESS;
  return VALIDATION_FAILED;
}
static int validate_interface_mode(char *mode) {
  if (strcmp(mode, "access") == 0 || strcmp(mode, "ACCESS") == 0 ||
      strcmp(mode, "trunk") == 0 || strcmp(mode, "TRUNK") == 0)
    return VALIDATION_SUCCESS;
  printf("Error : Invalid interface mode \"%s\" provided.\n", mode);
  return VALIDATION_FAILED;
}
static int validate_mask_handler(char *mask) {
...
static int
exit_handler(param_t *param,           // parameter passed to handler call back
             ser_buff_t *tlv_buff,     // tlv structure described param
             op_mode enable_or_disable // is command enable or disable function
) {
  tlv_struct_t *tlv = NULL;
...
void nw_init_cli() {
  ...
  init_param(&topology, CMD, "topology", show_nw_topology_handler, 0, INVALID,
             0, "Dump Completed Network Topology");
  libcli_register_param(show, &topology);
  set_param_cmd_code(&topology, CMDCODE_SHOW_NW_TOPOLOGY);
  // saved
  {
    static param_t saved;
    init_param(&saved, CMD, "saved", show_saved_topologies_handler, 0, INVALID,
               0, "Help : Show topologies saved");
    libcli_register_param(&topology, &saved);
    set_param_cmd_code(&saved, CMDCODE_SHOW_TOPOLOGIES_SAVED);
    // topology-id
    {
      static param_t topology_id;
      init_param(&topology_id, LEAF, 0, show_saved_topologies_handler,
                 validate_topology_id, STRING, "topology-id",
                 "Help : Topology id");
...
// node
{
  ...
  //<node-name>
  {
    ...
    init_param(&node_name, LEAF, 0, show_node_handler, 0, STRING, "node-name",
               "Help : Node name");
    ...
    {
      static param_t arp;
      init_param(&arp, CMD, "arp", show_arp_handler, 0, INVALID, 0,
    ...
    // mac-table
    {
      ...
      init_param(&mac_table, CMD, "mac-table", show_mac_handler, 0, INVALID, 0,
    ...
    // debug
    {
      ...
      init_param(&debug_status, CMD, "debug", show_debug_handler, 0, INVALID, 0,
    ...
    // interface
    {
      ...
      init_param(&interface, CMD, "interface", show_node_interfaces, 0, INVALID,
                 0, "node interface");
      ...
      // interface-name
      {
        ...
        init_param(&interface_name, LEAF, 0, 0, 0, STRING, "interface-name",
                   "Help : Interface Name");
        libcli_register_param(&interface, &interface_name);
        // mode
        {
          static param_t mode;
          init_param(&mode, CMD, "mode", show_mode_handler, 0, INVALID, 0,
        ...
        // vlans
        {
          ...
          init_param(&vlans, CMD, "vlans", show_vlans_handler, 0, INVALID, 0,
    ...
    // route
    {
      ...
      init_param(&route, CMD, "route", show_route_handler, 0, INVALID, 0,
...
{
  static param_t load;
  init_param(&load, CMD, "load", 0, 0, INVALID, 0,
             "Help : Load Network Topology");
...
// create
{
  ...
  init_param(&create, CMD, "create", 0, 0, INVALID, 0,
             "Help : Create Network Topology");
  libcli_register_param(&topology, &create);
  // topology-id
  {
    static param_t topology_id;
    init_param(&topology_id, LEAF, 0, create_topology_handler, 0, STRING,
               "topology-name", "Help : Name of topology to create");
...
// node
{
  ...
  {
    static param_t node_name;
    init_param(&node_name, LEAF, 0, 0, 0, STRING, "node-name",
               "Help : Node name");
    // debug
    {
      libcli_register_param(&node, &node_name);
      ...
      // status
      {
        ...
        init_param(&status, LEAF, 0, config_debug_handler,
                   validate_debug_status, STRING, "status",
    ...
    {
      static param_t interface;
      init_param(&interface, CMD, "interface", 0, 0, INVALID, 0,
                 "node interface");
      libcli_register_param(&node_name, &interface);
      // interface-name
      {
        ...
        {
          static param_t mode;
          init_param(&mode, CMD, "mode", 0, 0, INVALID, 0,
                     "Show Mode of Interface on Node");
          libcli_register_param(&interface_name, &mode);
          // intf_mode
          {
            static param_t intf_mode;
            init_param(&intf_mode, LEAF, 0, mode_set_handler,
                       validate_interface_mode, STRING, "intf_mode",
        ...
        {
          static param_t vlans;
          init_param(&vlans, CMD, "vlans", 0, 0, INVALID, 0,
                     "Show Vlan membership of Interface on Node");
          libcli_register_param(&interface_name, &vlans);
          // add
          {
            ...
            init_param(&add, CMD, "add", 0, 0, INVALID, 0,
                       "Help : Add Vlan to Interface ");
            libcli_register_param(&vlans, &add);
            // vlan_id
            {
              static param_t vlan_id;
              init_param(&vlan_id, LEAF, 0, add_vlan_handler, 0, INT, "vlan-id",
                         "Help : Vlan ID");
              libcli_register_param(&add, &vlan_id);
              set_param_cmd_code(&vlan_id,
                                 CMDCODE_CONFIG_NODE_INTERFACE_VLAN_ADD);
            }
          }
          // remove
          {
            static param_t remove;
            init_param(&remove, CMD, "remove", 0, 0, INVALID, 0,
                       "Help : remove Vlan From Interface ");
            libcli_register_param(&vlans, &remove);
            // vlan_id
            {
              static param_t vlan_id;
              init_param(&vlan_id, LEAF, 0, remove_vlan_handler, 0, INT,
                         "vlan-id", "Help : Vlan ID");
              libcli_register_param(&remove, &vlan_id);
              set_param_cmd_code(&vlan_id,
                                 CMDCODE_CONFIG_NODE_INTERFACE_VLAN_REMOVE);
            }
          }
        }
      }
    ...
    // route
    {
      ...
      {
        static param_t dest;
        init_param(&dest, LEAF, 0, 0, 0, IPV4, "dest",
                   "Help : Destination Network");
        libcli_register_param(&route, &dest);
        // mask
        {
          static param_t mask;
          init_param(&mask, LEAF, 0, 0, validate_mask_handler, INT, "mask",
                     "Help : Net Mask (0 - 32 )");
          ...
          // gw
          {
            ...
            {
              static param_t oif;
              init_param(&oif, LEAF, 0, config_route_handler, 0, STRING, "oif",
...
//<node-name>
{
  ...
  // resolve_arp
  {
    ...
    {
      static param_t ip_address;
      init_param(&ip_address, LEAF, 0, resolve_arp_handler, validate_ip_address,
                 STRING, "ip-address", "Help : IP Address");
...
{
  static param_t exit;
  init_param(&exit,                  /* pointer to static param_t variable*/
             CMD,                    /* CMD|LEAF*/
             "exit",                 /* <command name> | NULL*/
             exit_handler,           /* Callback field*/
             0,                      /* NULL | <callback ptr>*/
             INVALID,                /* INVALID | leaf type*/
             0,                      /* NULL, <STRING>*/
             "Help : Exit simulator" /* Help String*/
  );
  libcli_register_param(root, &exit);
  set_param_cmd_code(&exit, CMDCODE_EXIT);
}
}