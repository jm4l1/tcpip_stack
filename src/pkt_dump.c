...
void arp_pkt_dump(char *pkt) {
  ...
  printf("\tSrc Mac: %02x:%02x:%02x:%02x:%02x:%02x\n",
         arp_packet->src_mac.mac[0], arp_packet->src_mac.mac[1],
         arp_packet->src_mac.mac[2], arp_packet->src_mac.mac[3],
         arp_packet->src_mac.mac[4], arp_packet->src_mac.mac[5]);
  printf("\tSrc IP: %s\n", arp_src_ip);
  printf("\tTarget Mac: %02x:%02x:%02x:%02x:%02x:%02x\n",
         arp_packet->dst_mac.mac[0], arp_packet->dst_mac.mac[1],
         arp_packet->dst_mac.mac[2], arp_packet->dst_mac.mac[3],
         arp_packet->dst_mac.mac[4], arp_packet->dst_mac.mac[5]);
  printf("\tTarget IP: %s\n", arp_target_ip);
}
void ip_pkt_dump(char *pkt) {
  ...
  printf("\tFlags: %hhu %hhu %hhu\n", ip_packet->reserved, ip_packet->DF,
         ip_packet->MF);
...
void pkt_dump(ethernet_frame_t *eth_frame, unsigned int pkt_size) {
  uint16_t type;
  char payload[MAX_ETH_PAYLOAD];
  printf("<Ethernet>\n");
  printf("Dst Mac: %02x:%02x:%02x:%02x:%02x:%02x\n", eth_frame->dest_mac.mac[0],
         eth_frame->dest_mac.mac[1], eth_frame->dest_mac.mac[2],
         eth_frame->dest_mac.mac[3], eth_frame->dest_mac.mac[4],
         eth_frame->dest_mac.mac[5]);
  printf("Src Mac: %02x:%02x:%02x:%02x:%02x:%02x\n", eth_frame->src_mac.mac[0],
  ...
  if (vlan_eth_frame) {
    ...
    memcpy(payload, vlan_eth_frame->payload,
           pkt_size - VLAN_ETH_HDR_SIZE_EXCL_PAYLOAD);
    printf("<VLAN TAGGED>\n");
