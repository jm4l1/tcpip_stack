...
extern void demote_pkt_to_layer2(node_t *node, uint32_t next_hop_ip,
                                 interface_t *intf, char *pkt,
                                 uint32_t pkt_size, uint16_t protocol_number);
extern char *pkt_buffer_shift_right(char *pkt, unsigned int pkt_size,
                                    unsigned int total_buffer_size);
extern void ip_pkt_dump(char *pkt);
extern void icmp_pkt_dump(char *pkt);
...
void init_rt_table(route_table_t **route_table) {
  *route_table = calloc(1, sizeof(route_table_t));
  ...
  if (FALSE)
    printf("[init_rt_table] Info - Route Table Initialized\n");
};
void rt_table_add_direct_route(route_table_t *route_table, char *dst,
                               uint8_t mask) {
...
void rt_dump_table(route_table_t *route_table) {
  ...
  ITERATE_GLTHREAD_BEGIN(&route_table->route_entries, curr) {
    ...
    printf("Dst: %2s/%-4hhu %-5s , GW: %s, OIF : %s\n", l3route->dest,
           l3route->mask,
           (l3route->is_direct ? "directly connected" : "remote subnet"),
           l3route->gw, l3route->oif);
  }
  ITERATE_GLTHREAD_END(&route_table->route_entries, curr)
};
void rt_table_add_route(route_table_t *route_table, char *dst, uint8_t mask,
                        char *gw, char *oif_name) {
...
layer3_route_t *l3rib_lookup_lpm(route_table_t *route_table, uint32_t dest) {
  ...
  ITERATE_GLTHREAD_BEGIN(&route_table->route_entries, curr) {
    ...
    // printf("[l3rib_lookup_lpm] - destination ip %s , subnet calculated %s ,
    // l3_route mask %hu ,l3_route dest  %s  , match check %d\n",
    //         dest_ip_str , subnet , l3_route->mask , l3_route->dest ,
    //         strcmp(subnet, l3_route->dest) );
  ...
  // printf("[l3rib_lookup_lpm] - Dest %s , Subnet Calculated %s ,  Subnet
  // Matched %s \n", dest_ip_str , subnet ,  lpm_l3_route->dest);
  return lpm_l3_route;
}
static bool_t l3_is_direct_route(layer3_route_t *l3_route) {
...
bool_t is_layer3_local_delivery(node_t *node, unsigned int dst_ip) {
  ...
  if (node->debug_status == DEBUG_ON)
    printf("[is_layer3_local_delivery] - Node %s - checking local delivery\n",
           node->node_name);
  convert_ip_from_int_to_str(dst_ip, ip_addr);
  if (strcmp(NODE_LO_ADDR(node), ip_addr) == 0) {
    if (node->debug_status == DEBUG_ON)
      printf("[is_layer3_local_delivery] - Node %s - IP is a loopback\n",
             node->node_name);
    return TRUE;
  }
  if (node->debug_status == DEBUG_ON)
    printf("[is_layer3_local_delivery] - Node %s - IP is not Local Loopback\n",
           node->node_name);
  interface_t *intf;
  for (int i = 0; i < MAX_INTF_PER_NODE; i++) {
    ...
    if (node->debug_status == DEBUG_ON)
      printf("[is_layer3_local_delivery] - Node %s - Comparing IP %s and %s\n",
             node->node_name, IF_IP(intf), ip_addr);
    if (strcmp(IF_IP(intf), ip_addr) == 0) {
      if (node->debug_status == DEBUG_ON)
        printf("[is_layer3_local_delivery] - Node %s - IP is a local address\n",
               node->node_name);
      return TRUE;
    }
  }
  if (node->debug_status == DEBUG_ON)
    printf(
        "[is_layer3_local_delivery] - Node %s - IP is not a local delivery\n",
        node->node_name);
  return FALSE;
}
static icmp_pkt_t *make_icmp_time_exceeded(ip_hdr_t *ip_hdr, uint8_t code) {
...
static icmp_pkt_t *make_icmp_dest_unreacheable(ip_hdr_t *ip_hdr, uint8_t code) {
  icmp_hdr_t *header = calloc(1, sizeof(icmp_hdr_t));
...
void send_icmp_time_exceeded(node_t *node, ip_hdr_t *ip_hdr) {
  if (node->debug_status == DEBUG_ON)
    printf("[send_icmp_time_exceeded] - creating ICMP Time Exceeded\n");
  icmp_pkt_t *icmp_time_exceeded =
      make_icmp_time_exceeded(ip_hdr, ICMP_CODE_TIME_EXCEEDED_TTL);
  demote_pkt_to_layer3(node, (char *)icmp_time_exceeded, sizeof(icmp_pkt_t),
                       ICMP_PROTO, ip_hdr->src);
  if (node->debug_status == DEBUG_ON)
    printf("[send_icmp_time_exceeded] - Time Exceeded Packet sent\n");
}
void send_icmp_dest_unreacheable(node_t *node, ip_hdr_t *ip_hdr) {
  if (node->debug_status == DEBUG_ON)
    printf(
        "[send_icmp_dest_unreacheable] - creating Destination Unreachable\n");
  icmp_pkt_t *icmp_dest_unreacheable =
      make_icmp_dest_unreacheable(ip_hdr, ICMP_CODE_DEST_UNREACH_NET);
  demote_pkt_to_layer3(node, (char *)icmp_dest_unreacheable, sizeof(icmp_pkt_t),
                       ICMP_PROTO, ip_hdr->src);
  if (node->debug_status == DEBUG_ON)
    printf("[send_icmp_dest_unreacheable] - Destination Unreachable Packet "
           "sent\n");
}
static void layer3_pkt_receive_from_top(node_t *node, char *pkt,
                                        uint32_t data_size,
  ...
  uint32_t src_addr;
  interface_t *oif;
  layer3_route_t *l3_rt =
      l3rib_lookup_lpm(node->node_nw_prop.route_table, dest_ip_address);
  char *l3_pkt = NULL;
  uint32_t new_pkt_size;
  memset(dest_string, 0, INET_ADDRSTRLEN);
  inet_ntop(AF_INET, &dest_ip_address, dest_string, INET_ADDRSTRLEN);
  // printf("destination ip is %s\n", dest_string);     //src address chosen to
  // be that of the next hop interface
  ip_hdr.proto = protocol_number;
  ip_hdr.ihl = 5;
  ip_hdr.dest = dest_ip_address;
  if (!l3_rt) {
    if (node->debug_status == DEBUG_ON)
      printf("[layer3_pkt_receive_from_top] - No route to destination!\n");
    if (ip_hdr.src == 0) {
  ...
  if (node->debug_status == DEBUG_ON)
    printf("[layer3_pkt_receive_from_top] - Route found to %s via %s!\n",
  ...
  if (l3_is_direct_route(l3_rt)) {
    ...
    if (!oif) {
      ...
      ip_hdr.src = src_addr;
      if (node->debug_status == DEBUG_ON)
        printf("[layer3_pkt_receive_from_top] - Direct Route found, will use "
               "Loopback\n");
    } else {
    ...
    if (node->debug_status == DEBUG_ON)
      printf("[layer3_pkt_receive_from_top] - Packet will be forwarded, will "
             "use OIF %s\n",
             oif->if_name);
    inet_pton(AF_INET, IF_IP(oif), &src_addr);
    ip_hdr.src = src_addr;
  ...
  if (node->debug_status == DEBUG_ON)
    printf("[layer3_pkt_receive_from_top] - IP header created\n");
  char *shifted_pkt_buffer =
      pkt_buffer_shift_right(l3_pkt, new_pkt_size, MAX_PACKET_BUFFER_SIZE);

  if (!l3_is_direct_route(l3_rt)) {
    if (node->debug_status == DEBUG_ON)
      printf("[layer3_pkt_receive_from_top] - route is not direct route\n");
    inet_pton(AF_INET, l3_rt->gw, &next_hop_add);
    if (node->debug_status == DEBUG_ON)
      printf("[layer3_pkt_receive_from_top] - sending out OIF %s (%s)\n",
             oif->if_name, oif->intf_nw_props.ip_add.ip_addr);
    demote_pkt_to_layer2(node, next_hop_add, oif, shifted_pkt_buffer,
                         new_pkt_size, IP_PROTO);
  } else {
    if (node->debug_status == DEBUG_ON)
      printf("[layer3_pkt_receive_from_top] - route direct route\n");
    demote_pkt_to_layer2(node, dest_ip_address, 0, shifted_pkt_buffer,
                         new_pkt_size, IP_PROTO);
  }
  free(l3_pkt);
  return;
}
void demote_pkt_to_layer3(node_t *node, char *pkt, uint32_t data_size,
                          uint8_t protocol_number, uint32_t dest_ip_address) {
  layer3_pkt_receive_from_top(node, pkt, data_size, protocol_number,
                              dest_ip_address);
}
...
void send_icmp_echo_reply(node_t *node, uint16_t identifier, uint16_t seq_num,
                          uint32_t reply_data, uint32_t dest_ip) {
  if (node->debug_status == DEBUG_ON)
    printf("[send_icmp_echo_reply] - creating echo reply\n");
  icmp_pkt_t *icmp_echo_reply =
      make_icmp_echo_reply(identifier, seq_num, reply_data);
  if (node->debug_status == DEBUG_ON)
    printf("[send_icmp_echo_reply] - sending Packet with sequence number %hu\n",
           seq_num);
  demote_pkt_to_layer3(node, (char *)icmp_echo_reply, sizeof(icmp_pkt_t),
                       ICMP_PROTO, dest_ip);
  if (node->debug_status == DEBUG_ON)
...
void send_icmp_echo_message(node_t *node, uint16_t identifier, uint16_t seq_num,
                            uint32_t dest_ip) {
  if (node->debug_status == DEBUG_ON)
    printf("[send_icmp_echo_message] - creating echo message\n");
  icmp_pkt_t *icmp_echo_message = make_icmp_echo_message(identifier, seq_num);
  if (node->debug_status == DEBUG_ON)
    printf(
        "[send_icmp_echo_message] - sending Packet with sequence number %hu\n",
        seq_num);
  demote_pkt_to_layer3(node, (char *)icmp_echo_message, sizeof(icmp_pkt_t),
                       ICMP_PROTO, dest_ip);
  if (node->debug_status == DEBUG_ON)
...
static void process_icmp_pkt(node_t *node, ip_hdr_t *ip_hdr) {
  ...
  case ICMP_TYPE_ECHO_REPLY:
    ...
    printf("Node %s : %hu bytes received from %s: icmp_seq=%hu ttl=%hhu\n",
           node->node_name, IP_HDR_PAYLOAD_SIZE(ip_hdr), src_add,
           icmp_pkt->header.seq_num, ip_hdr->TTL);
    break;
  ...
  case ICMP_TYPE_ECHO_MESSAGE:
    ...
    send_icmp_echo_reply(node, icmp_pkt->header.identifier,
                         icmp_pkt->header.seq_num, reply_data, ip_hdr->src);
    break;
  case ICMP_TYPE_TIME_EXCEEDED:
    if (node->debug_status == DEBUG_ON)
      printf("[process_icmp_pkt] - Processing ICMP Packet\n");
    memset(dest_add, 0, INET_ADDRSTRLEN);
    convert_ip_from_int_to_str(icmp_pkt->data, dest_add);
    switch (icmp_pkt->header.code) {
...
static void layer3_ip_pkt_recv_from_layer2(node_t *node, interface_t *intf,
  ...
  if (node->debug_status == DEBUG_ON)
    printf("[layer3_ip_pkt_recv_from_layer2] - Node %s - Info : Looking up ip "
           "%s in route table\n",
           node->node_name, dest_ip_add);
  // lookup route
  layer3_route_t *l3_rt =
      l3rib_lookup_lpm(node->node_nw_prop.route_table, dest_ip);
  // route not found
  if (!l3_rt) {
    if (node->debug_status == DEBUG_ON)
      printf("[layer3_ip_pkt_recv_from_layer2] - Node %s - Info : No route "
             "found for ip %s\n",
             node->node_name, dest_ip_add);
    send_icmp_dest_unreacheable(node, ip_hdr);
    return;
  ...
  if (node->debug_status == DEBUG_ON)
    printf("[layer3_ip_pkt_recv_from_layer2] - Node %s - Info : Route found "
           "for ip %s\n",
           node->node_name, dest_ip_add);
  // route found
  if (l3_is_direct_route(l3_rt)) { // is route local
    if (node->debug_status == DEBUG_ON)
      printf("[layer3_ip_pkt_recv_from_layer2] - Node %s - Info Route Matched "
             "Direct Route\n",
             node->node_name);
    if (is_layer3_local_delivery(node, dest_ip)) { // local delivery
      if (node->debug_status == DEBUG_ON)
        printf("[layer3_ip_pkt_recv_from_layer2] - Node %s - Local Delivery\n",
               node->node_name);
      switch (ip_hdr->proto) {
      case ICMP_PROTO:
    ...
    } else { // directly connected host delivery
      if (node->debug_status == DEBUG_ON)
        printf("[layer3_ip_pkt_recv_from_layer2] - Node %s - Connected host "
               "Delivery\n",
               node->node_name);
      demote_pkt_to_layer2(node, dest_ip, NULL, (char *)ip_hdr, pkt_size,
                           IP_PROTO);
    }
    return;
  } else { // forwading case
    if (node->debug_status == DEBUG_ON)
      printf("[layer3_ip_pkt_recv_from_layer2] - Node %s  : Packet to be "
             "forwarded\n",
             node->node_name);
    --ip_hdr->TTL;
    if ((ip_hdr->TTL) == 0) {
    ...
    inet_pton(AF_INET, l3_rt->gw, &next_hop_add);
    demote_pkt_to_layer2(node, next_hop_add,
                         get_node_if_by_name(node, l3_rt->oif), (char *)ip_hdr,
                         pkt_size, IP_PROTO);
  }
}
static void _layer3_pkt_recv_from_layer2(node_t *node, interface_t *intf,
                                         char *pkt, uint32_t pkt_size,
                                         uint16_t protocol_type) {
  switch (protocol_type) {
  case IP_PROTO: {
    if (node->debug_status == DEBUG_ON)
      printf("[_layer3_pkt_recv_from_layer2] -  Protocol Type  0x%x Received , "
             "Forwarding to IP Handler\n",
             protocol_type);
  ...
  default:
    if (node->debug_status == DEBUG_ON)
      printf("[_layer3_pkt_recv_from_layer2] - Unknown Protocol Type  0x%x , "
             "Dropping Packet\n",
             protocol_type);
    break;
  }
}
void promote_pkt_to_layer3(node_t *node, interface_t *recv_intf, char *payload,
                           uint32_t app_data_size, uint16_t protocol_number) {
  _layer3_pkt_recv_from_layer2(node, recv_intf, payload, app_data_size,
                               protocol_number);
}
