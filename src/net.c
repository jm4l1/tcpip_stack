...
extern void rt_table_add_direct_route(route_table_t *route_table, char *dst,
                                      uint8_t mask);
...
void interface_assign_mac_address(interface_t *interface) {
  ...
  memcpy(IF_MAC(interface), (char *)&hash_code_val, sizeof(hash_code_val));
}
bool_t node_set_loopback_address(node_t *node, char *ip_addr) {
  node->node_nw_prop.is_lb_addr_config = TRUE;
  strcpy(node->node_nw_prop.lb_addr.ip_addr, ip_addr);
  rt_table_add_direct_route(node->node_nw_prop.route_table, ip_addr,
                            (uint8_t)32);
  return TRUE;
}
bool_t node_set_intf_ip_address(node_t *node, char *local_if, char *ip_addr,
                                char mask) {
  ...
  rt_table_add_direct_route(node->node_nw_prop.route_table, dest,
                            (uint8_t)mask);
  return TRUE;
}
...
void dump_nw_graph(graph_t *graph) {
  ...
  ITERATE_GLTHREAD_END(&topology->node_list, curr);
}
void dump_nw_node(node_t *node) {
  printf("Node Name : %s\n", node->node_name);
  char *lo_addr = strcat(NODE_LO_ADDR(node), "/32")
...
void dump_nw_interface(interface_t *intf) {
  ...
  printf("\tNbr Node %s, Local Node : %s , cost = %d\n", nbr->node_name,
         intf->if_name, intf->link->cost);
  printf("\tIP Addr : %s(%s/%d )  MAC : %02x:%02x:%02x:%02x:%02x:%02x \n",
         IF_IP(intf), ip_subnet, mask, IF_MAC(intf)[0], IF_MAC(intf)[1],
         IF_MAC(intf)[2], IF_MAC(intf)[3], IF_MAC(intf)[4], IF_MAC(intf)[5]);
}
...
void node_set_intf_l2_mode(node_t *node, char *if_name,
                           intf_l2_mode_t intf_l2_mode) {
  interface_t *intf = get_node_if_by_name(node, if_name);
  // not a valid interface
  if (!intf) {
    printf("[node_set_intf_l2_mode] Error : , Node : %s - Unable to set L2 "
           "mode. %s not valid IF name\n",
  ...
  if (curr_l2_mode == intf_l2_mode)
    return;
  // interface configured in L3 mode
  if (IS_INTF_L3_MODE(intf)) {
    printf("[node_set_intf_l2_mode] Unable to configure interface %s in %s "
           "mode. Interface has IP configured\n",
  ...
  if (IF_L2_MODE(intf) == TRUNK && intf_l2_mode == ACCESS) {
    printf("[node_set_intf_l2_mode] Info : Setting Interface from trunk to "
           "acces mode , removing vlan members\n");
    memset((uint16_t *)(intf->intf_nw_props.vlans) + 1, 0,
           sizeof(uint16_t) * (MAX_VLAN_MEMBERSHIP - 1));
  }
  IF_L2_MODE(intf) = intf_l2_mode;
}
void add_vlan_to_trunk_membership(interface_t *intf, uint16_t vlan_id) {
  uint16_t slot = 0;
  int32_t empty_slot = -1;
  for (; slot < MAX_VLAN_MEMBERSHIP; slot++) {
    if (vlan_id == intf->intf_nw_props.vlans[slot]) {
      printf("[add_vlan_to_trunk_membership] Info : Node %s - Interface %s is "
             "already a member of VLAN %hu\n",
  ...
  if (empty_slot != -1) {
    printf("[add_vlan_to_trunk_membership] Info : Node %s - Adding VLAN %hu to "
           "Interface %s \n",
           intf->if_name, vlan_id, intf->att_node->node_name);
    intf->intf_nw_props.vlans[empty_slot] = vlan_id;
    return;
  }
  printf("[add_vlan_to_trunk_membership] Error : Node %s - Unable to set VLAN "
         "on IF %s , Max VLAN membership  %hu reached \n",
         intf->att_node->node_name, intf->if_name, MAX_VLAN_MEMBERSHIP);
}
void interface_set_vlan(node_t *node, interface_t *intf, uint16_t vlan_id) {
  if (IS_INTF_L3_MODE(intf)) {
    printf("[interface_set_vlan] Error : Node %s - Unable to set VLAN on IF %s "
...
void node_set_intf_vlan_membership(node_t *node, char *if_name,
                                   uint16_t vlan_id) {
  interface_t *intf = get_node_if_by_name(node, if_name);
  if (!intf) {
    printf("[node_set_intf_vlan_membership] Error : Node : %s - Unable to set "
           "VLAN Membership on Interface. %s not valid IF name\n",
           node->node_name, if_name);
    return;
  }
  interface_set_vlan(node, intf, vlan_id);
}
void node_remove_vlan_membership(node_t *node, char *if_name,
                                 uint16_t vlan_id) {
  interface_t *intf = get_node_if_by_name(node, if_name);
  if (!intf) {
    printf("[node_remove_vlan_membership] Error : Node : %s - Unable to remove "
           "VLAN Membership on Interface. %s not valid IF name\n",
           node->node_name, if_name);
    return;
  }
  if (IF_L2_MODE(intf) == L2_MODE_UNKNOWN) {
    printf("[node_remove_vlan_membership] Error : Node : %s - Unable to remove "
           "VLAN Membership on Interface %s , Unknown L2 Mode\n",
           node->node_name, if_name);
    return;
  }
  uint16_t slot = 0;
  for (; slot < MAX_VLAN_MEMBERSHIP; slot++) {
    ...
    if (vlan == vlan_id) {
      printf("[node_remove_vlan_membership] Info : Node :%s - Removing VLAN "
             "%hu from IF %s membership\n",
             node->node_name, vlan_id, if_name);
      memset((uint16_t *)(intf->intf_nw_props.vlans) + slot, 0,
             sizeof(uint16_t));
      return;
    }
  }
  printf(
      "[node_remove_vlan_membership] Error : Node : %s - Unable to remove VLAN "
      "Membership on Interface %s , VLAN %hu is not a member of interface\n",
      node->node_name, if_name, vlan_id);
  return;
}
uint16_t get_access_intf_operating_vlan_id(interface_t *intf) {
  if (IF_L2_MODE(intf) != ACCESS) {
    printf("[get_access_intf_operating_vlan_id] Error : Unable to get access "
           "Vlan , Interface %s is not in access mode \n",
           intf->if_name);
    return 0;
  }
...
bool_t is_trunk_interface_vlan_member(interface_t *intf, uint16_t vlan_id) {
  if (IF_L2_MODE(intf) != TRUNK) {
    printf("[is_trunk_interface_vlan_member] Error :  Interface %s is not in "
           "trunk mode \n",
           intf->if_name);
    return 0;
  }
