...
extern void l2_switch_recv_frame(interface_t *intf, char *pkt,
                                 uint32_t pkt_size);
extern void promote_pkt_to_layer3(node_t *node, interface_t *recv_intf,
                                  char *payload, uint32_t app_data_size,
                                  uint16_t protocol_number);
extern bool_t is_layer3_local_delivery(node_t *node, unsigned int dst_ip);
extern void pkt_dump(ethernet_frame_t *eth_frame, unsigned int pkt_size);
...
void init_arp_table(arp_table_t **arp_table) {
  *arp_table = calloc(1, sizeof(arp_table_t));
  ...
  if (FALSE)
    printf("[init_arp_table] Info - Arp Table Initialized\n");
}
bool_t arp_table_entry_add(arp_table_t *arp_table, arp_entry_t *arp_entry) {
  ...
  arp_entry_t *arp_entry_old =
      arp_table_lookup(arp_table, arp_entry->ip_addr.ip_addr);
  if (arp_entry_old) {
    // if entry is already in table
    if (memcmp(arp_entry_old, arp_entry, sizeof(arp_entry_t)) == 0)
      return FALSE;
...
void arp_table_dump(arp_table_t *arp_table) {
  ...
  ITERATE_GLTHREAD_BEGIN(&arp_table->arp_entries, curr) {
    ...
    printf("IP : %-16s\t MAC : %x:%x:%x:%x:%x:%x\t OIF : %s\n",
           arp_entry->ip_addr.ip_addr, arp_entry->mac_addr.mac[0],
           arp_entry->mac_addr.mac[1], arp_entry->mac_addr.mac[2],
           arp_entry->mac_addr.mac[3], arp_entry->mac_addr.mac[4],
...
static void add_arp_pending_entry(arp_entry_t *arp_entry,
                                  arp_process_fun callback, char *pkt,
                                  uint32_t pkt_size) {
...
void send_arp_broadcast_request(node_t *node, interface_t *oif, char *ip_addr) {
  ...
  if (!oif) {
    ...
    if (!oif) {
      if (node->debug_status == DEBUG_ON)
        printf("[send_arp_broadcast_request] Info : node %s , No eligble "
               "interface found for IP %s\n",
               node->node_name, ip_addr);
      return;
    }
    if (strcmp(ip_addr, IF_IP(oif)) == 0) {
      if (node->debug_status == DEBUG_ON)
        printf("[send_arp_broadcast_request] Error : node %s , Attempting to "
               "send Arp for local IP %s on interface OIF : %s \n",
  ...
  for (int idx = 0; idx < 6; idx++) {
    eth_frame->src_mac.mac[idx] = if_mac[idx];
  }
  ...
  for (int idx = 0; idx < sizeof(mac_addr_t); idx++) {
    arp_broadcast->src_mac.mac[idx] = if_mac[idx];
  }
  arp_broadcast->src_ip = htonl(src_ip_l);
  for (int idx = 0; idx < sizeof(mac_addr_t); idx++) {
    arp_broadcast->dst_mac.mac[idx] = 0;
  }
  ...
  *eth_fcs = 0;
  ...
  if (node->debug_status == DEBUG_ON)
    printf("[send_arp_broadcast_request] Info : sending Arp Broadcast from "
           "node %s on to IP %s on OIF: %s\n",
           node->node_name, ip_addr, oif->if_name);
  send_pkt_out((char *)eth_frame, eth_frame_size, oif);

  free(eth_frame);
}
static void pending_arp_processing_callback_function(
    node_t *node, interface_t *oif, arp_entry_t *arp_entry,
    arp_pending_entry_t *arp_pending_entry) {
  ...
  set_common_eth_fcs(eth_frame,
                     pkt_size - get_eth_hdr_size_excl_payload(eth_frame), 0);
  send_pkt_out((char *)eth_frame, pkt_size, oif);
}
static void send_arp_reply_msg(ethernet_frame_t *eth_frame_in,
                               interface_t *oif) {
  arp_packet_t *arp_request = (arp_packet_t *)eth_frame_in->payload;
  ethernet_frame_t *eth_frame_out = calloc(1, MAX_PACKET_BUFFER_SIZE);
  char *if_mac = IF_MAC(oif);

  memcpy(eth_frame_out->dest_mac.mac, eth_frame_in->src_mac.mac,
         sizeof(mac_addr_t));
  ...
  memcpy(arp_reply->dst_mac.mac, arp_request->src_mac.mac, sizeof(mac_addr_t));
  ...
  *eth_fcs = 0;
  ...
  char *shifted_pkt_buffer = pkt_buffer_shift_right(
      (char *)eth_frame_out, total_pkt_size, MAX_PACKET_BUFFER_SIZE);

  if (oif->att_node->debug_status == DEBUG_ON)
    printf("[send_arp_reply_msg] Info : %s - Sending Arp Reply\n",
...
static void process_arp_broadcast_request(node_t *node, interface_t *iif,
  ...
  if (node->debug_status == DEBUG_ON)
    printf("[process_arp_broadcast_request] Info : Incoming ARP msg Revcd on "
           "IF %s of Node %s\n",
  ...
  if (strcmp(local_if_ip, target_ip) != 0) {
    if (node->debug_status == DEBUG_ON)
      printf("[process_arp_broadcast_request] Info :  Discarding ARP msg Recvd "
             "on IF %s of Node %s for IP %s\n",
             iif->if_name, node->node_name, target_ip);
    return;
  }
  send_arp_reply_msg(eth_frame, iif);
}
static void process_arp_reply_message(node_t *node, interface_t *iif,
                                      ethernet_frame_t *eth_frame) {
  if (node->debug_status == DEBUG_ON)
    printf("[process_arp_reply_message] Info : %s - received arp reply\n",
           node->node_name);

  arp_table_update_from_arp_reply(node->node_nw_prop.arp_table,
                                  (arp_packet_t *)eth_frame->payload, iif);
}
// L2 APIs
static void promote_pkt_to_layer2(node_t *node, interface_t *intf,
                                  ethernet_frame_t *eth_frame,
                                  uint32_t pkt_size) {
  ...
  case ARP_PACKET: {
    ...
    case ARP_REQUEST:
      if (node->debug_status == DEBUG_ON)
        printf("[promote_pkt_to_layer2] Info : %s - ARP Request received \n",
               node->node_name);
      process_arp_broadcast_request(node, intf, eth_frame);
      break;
    case ARP_REPLY:
      if (node->debug_status == DEBUG_ON)
        printf("[promote_pkt_to_layer2] Info : %s - ARP Reply received \n",
               node->node_name);
  ...
  case IP_PROTO: {
    if (node->debug_status == DEBUG_ON)
      printf("[promote_pkt_to_layer2] Info : %s - IP (0x%4x) Packet Received , "
             "sending to layer 3 \n",
             node->node_name, ethertype);
    promote_pkt_to_layer3(node, intf, get_ethernet_frame_payload(eth_frame),
                          pkt_size - get_eth_hdr_size_excl_payload(eth_frame),
                          ethertype);
  } break;
  default:
    if (node->debug_status == DEBUG_ON)
      printf("[promote_pkt_to_layer2] Info : %s - Unknown Ethertype 0x%4x \n",
             ethertype);
    ;
  }
}
static void l2_forward_ip_packet(node_t *node, uint32_t next_hop_ip, char *intf,
                                 ethernet_frame_t *eth_frame,
                                 uint32_t pkt_size) {
  if (node->debug_status == DEBUG_ON)
    printf("[l2_forward_ip_packet] - Node %s - starting packet forward.\n",
           node->node_name);
  ...
  if (!intf) {
    if (is_layer3_local_delivery(node, next_hop_ip)) {
      promote_pkt_to_layer3(node, 0, get_ethernet_frame_payload(eth_frame),
                            pkt_size - get_eth_hdr_size_excl_payload(eth_frame),
                            eth_frame->type);
      return;
    }
  ...
  if (oif)
    if (node->debug_status == DEBUG_ON)
      printf("[l2_forward_ip_packet] - Node %s - interface %s found\n",
...
void demote_pkt_to_layer2(node_t *node, uint32_t next_hop_ip, interface_t *intf,
                          char *pkt, uint32_t pkt_size,
                          uint16_t protocol_number) {
  if (node->debug_status == DEBUG_ON)
    printf("[demote_pkt_to_layer2] - protocol number %hu\n", protocol_number);
  ethernet_frame_t *eth_frame = ALLOC_ETH_HDR_WITH_PAYLOAD(pkt, pkt_size);
  eth_frame->type = IP_PROTO;

  if (node->debug_status == DEBUG_ON)
    printf("[demote_pkt_to_layer2] - eth proto 0x%4x\n", eth_frame->type);
  if (node->debug_status == DEBUG_ON)
    printf("[demote_pkt_to_layer2] - L2 frame created for IP Packet\n");
  l2_forward_ip_packet(node, next_hop_ip, intf->if_name, eth_frame,
                       ETH_HDR_SIZE_EXCL_PAYLOAD + pkt_size);
}
void layer2_frame_recv(node_t *node, interface_t *intf, char *pkt,
                       uint32_t pkt_size) {
  ethernet_frame_t *eth_frame = (ethernet_frame_t *)pkt;
  uint16_t vlan_to_tag;
  // Should the node accept Frame
  if (l2_frame_recv_qualify_on_interface(intf, eth_frame, &vlan_to_tag) ==
      FALSE) {
    if (node->debug_status == DEBUG_ON)
      printf("[layer2_frame_recv] Info : %s - L2 frame discarded\n",
             node->node_name);
    return;
  };
  // Is IIF in L3 mode
  if (IS_INTF_L3_MODE(intf)) {
    if (node->debug_status == DEBUG_ON)
      printf("[layer2_frame_recv] Info : %s - interface %s on node %s in %s "
             "mode , Sending to DL Layer\n",
             node->node_name, intf->if_name, intf->att_node->node_name,
             intf_l2_mode_str(intf->intf_nw_props.intf_l2_mode));
  ...
  // IS IIF in L2 mode
    ...
    if (vlan_to_tag) {
      pkt = (char *)tag_pkt_with_vlan_id((ethernet_frame_t *)pkt, pkt_size,
                                         vlan_to_tag, &new_pkt_size);
    }
    l2_switch_recv_frame(intf, pkt, vlan_to_tag ? new_pkt_size : pkt_size);
  }
}
ethernet_frame_t *tag_pkt_with_vlan_id(ethernet_frame_t *eth_frame,
  ...
  if (vlan_eth_frame) {
    printf("[tag_pkt_with_vlan_id] Info : Frame is tagged with VLAN ID %hu , "
           "Updating to %hu\n",
...
ethernet_frame_t *untag_pkt_with_vlan_id(ethernet_frame_t *eth_frame,
                                         uint32_t total_pkt_size,
                                         uint32_t *new_pkt_size) {
  uint32_t new_pkt_size_val = total_pkt_size;
  ...
  ethernet_frame_t *new_eth_frame =
      (ethernet_frame_t *)((char *)(vlan_eth_frame) + sizeof(vlan_8021q_tag_t));
